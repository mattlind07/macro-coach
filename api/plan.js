...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { userId, weightUnit = 'lb', goal, target_cal, protein_g, carbs_g, fat_g, maintenance, sex } = body
      if (!userId || !goal || target_cal == null) return res.status(400).json({ error: 'missing fields' })

      // upsert — one plan per user
      await sql`
        INSERT INTO plans (user_id, weight_unit, goal, target_cal, protein_g, carbs_g, fat_g, maintenance, sex)
        VALUES (${userId}, ${weightUnit}, ${goal}, ${target_cal}, ${protein_g}, ${carbs_g}, ${fat_g}, ${maintenance}, ${sex || null})
