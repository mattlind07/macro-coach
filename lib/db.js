// ============================================================
// db.js — Neon Postgres access + the recalibration engine
//
// Uses @neondatabase/serverless (the current recommended driver;
// @vercel/postgres is deprecated). The Neon Vercel integration
// injects the connection string as DATABASE_URL / POSTGRES_URL.
//
...
export function recalibrate(plan, weighIns) {
  ...
  if (days < 7) {
    return { applied: false, reason: 'Log at least a week apart — shorter spans are mostly water weight noise.' }
  }
