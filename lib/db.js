...
export function ensureSchema() {
  ...
  _schemaReady = (async () => {
    ...
    await sql`ALTER TABLE plans ADD COLUMN IF NOT EXISTS last_recalibrated_on DATE`
    await sql`
...
export function recalibrate(plan, weighIns) {
  ...
  const days = sorted.length >= 2
    ? (new Date(last.logged_on) - new Date(first.logged_on)) / MS_PER_DAY
    : 0

  // Need at least 7 entries spanning at least 7 days for a stable estimate.
  // Fewer points let single water-weight swings dominate the TDEE calculation.
  if (sorted.length < 7 || days < 7) {
    return {
      applied: false,
      reason: `Need at least 7 weigh-ins spanning 7 days for a reliable estimate — you have ${sorted.length} ${sorted.length === 1 ? 'entry' : 'entries'} over ${Math.round(days)} day${Math.round(days) === 1 ? '' : 's'}.`,
    }
  }

  // Enforce a weekly cadence: recalibrating daily re-anchors the ±250 cap to
  // noisy data, so each bad update becomes the new baseline.
  if (plan.last_recalibrated_on) {
    const daysSince = (new Date(last.logged_on) - new Date(plan.last_recalibrated_on)) / MS_PER_DAY
    if (daysSince < 7) {
      const daysLeft = Math.ceil(7 - daysSince)
      return {
        applied: false,
        reason: `Recalibration updates once a week for stability — check back in ${daysLeft} day${daysLeft === 1 ? '' : 's'}.`,
      }
    }
  }
  ...
  return {
    ...
    newPlan: {
      ...
      fat_g: fresh.fat_g,
      maintenance: estTDEE,
      last_recalibrated_on: dayKey(last.logged_on),
    },
    message,
  }
}
